...
def main():
    ...
    with open(infile, 'r') as mlst:
        for line in mlst:
            allele_list = line.split('\t')[0].split('_')
            loci.add('_'.join(allele_list[:-1]))
    num_loci = len(loci)
    list_loci = list(loci)
    list_loci.sort()
    d = defaultdict(dict)
    with open(infile, 'r') as mlst:
        for line in mlst:
            locus = '_'.join(line.split('\t')[0].split('_')[:-1])
            allele = line.split('\t')[0].split('_')[-1]
            accession = line.split('\t')[1]
            if  locus in d[line.split('\t')[1]]:
                allele_list = d[line.split('\t')[1]][locus]
                allele_list.append(allele)
                d[accession][locus] = allele_list
            else:    
                d[accession][locus] = [allele]
    '''
    for key in d:
        print(key)
        for v in d[key]:
